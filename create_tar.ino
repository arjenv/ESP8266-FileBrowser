...
void scan_dir(String path, File tarfile) {
  ...
  while (dir1.next()) {
    tarfiledate = dir1.fileCreationTime();
    localtime_r(&tarfiledate, &tarfilecreation);           // update the structure tm with the current time
    strcpy(pathfilename, (path + dir1.fileName()).c_str());
    if (dir1.isFile() && !dir1.fileName().endsWith(".tar")) { // skip any .tar files
      Serial.printf("File:\t%s\tSize:\t%u\t%02u-%02u-%04u\n", pathfilename, dir1.fileSize(), tarfilecreation.tm_mday, tarfilecreation.tm_mon+1, tarfilecreation.tm_year+1900);
      create_TAR(pathfilename, tarfile, dir1);
    }
    if (dir1.isDirectory()) {
      strcat(pathfilename, "/");
      Serial.printf("Dir:\t%s\tSize:\t%u\t%02u-%02u-%04u\n", pathfilename, dir1.fileSize(), tarfilecreation.tm_mday, tarfilecreation.tm_mon+1, tarfilecreation.tm_year+1900);
      create_TAR(pathfilename, tarfile, dir1);
      scan_dir(path+dir1.fileName()+"/", tarfile); //  Recursive
    }
  }
}

void create_TAR(char * filename, File tarfile, Dir dir) {
  /**********************************************************************************************************************************
  * USTAR Format
